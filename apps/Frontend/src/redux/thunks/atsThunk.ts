  ...
  async (data: AtsAnalysisPayload, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        `${INTEGRATION_BASE_URL}/integration/analyze-jd-match`,
        data,
        {
          headers: {
      ...
      if (resData && resData.success) {
        return {
          ...
          suggestions: Array.isArray(resData.suggestions)
            ? resData.suggestions
            : [],
          missingSkills: Array.isArray(resData.missingSkills)
            ? resData.missingSkills
            : [],
          topStrengths: Array.isArray(resData.topStrengths)
            ? resData.topStrengths
            : [],
          weaknesses: Array.isArray(resData.weaknesses)
            ? resData.weaknesses
            : [],
...
export const optimizeResume = createAsyncThunk(
  ...
  async (data: AtsAnalysisPayload, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        `${INTEGRATION_BASE_URL}/integration/optimize-resume`,
        data,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      return rejectWithValue(
        resData?.error || "Invalid response from optimizer"
      )
    } catch (error: any) {
      return rejectWithValue(
        error?.response?.data?.error ||
